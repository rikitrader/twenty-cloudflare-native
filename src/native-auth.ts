...
export const NATIVE_SESSION_TTL_SECONDS = 7 * 24 * 60 * 60;

export function nativeSessionExpiresAt(now = new Date()): string {
  return new Date(now.getTime() + NATIVE_SESSION_TTL_SECONDS * 1000).toISOString();
}

export function nativeSessionCookie(sessionId: string): string {
  return `twenty_session=${sessionId}; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=${NATIVE_SESSION_TTL_SECONDS}`;
}
...
export function nativeSessionToken(request: Request): string | null {
